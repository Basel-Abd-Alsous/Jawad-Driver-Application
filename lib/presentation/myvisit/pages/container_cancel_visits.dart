...
class ContainerCancelVisits extends StatelessWidget {
  const ContainerCancelVisits({super.key});

  @override
  Widget build(BuildContext context) {
    final local = AppLocalizations.of(context)!;
    return BlocProvider.value(
      value: sl<VisitCubit>()..getAllCanceledVisit(),
      child: BlocBuilder<VisitCubit, VisitState>(
        builder: (context, state) {
          return state.maybeWhen(
            loadingCancelVisits: () => const LoadingVisits(),
            loadedCancelVisits: (data, hasMore) {
                  ...
                  children: [
                    Expanded(
                      child: ListView.separated(
                        controller: context.read<VisitCubit>().scrollControllerCanceled,
            ...
            errorCancelVisits: (message) => Text(message),
